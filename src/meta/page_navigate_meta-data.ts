export default {
  Home: [
    {
      name: 'main',
      korean: '메인',
      url: 'http://localhost:3000',
    },
    { name: 'announce', korean: '공지', url: 'https://www.google.com' },
    { name: 'timetable', korean: '시간표', url: 'https://www.google.com' },
    { name: 'maps', korean: '편의시설', url: 'https://www.google.com' },
  ],
  User: [
    { name: 'score', korean: '성적', url: 'https://www.google.com' },
    { name: 'scolarship', korean: '장학금', url: 'https://www.google.com' },
    { name: 'enrollment', korean: '등록금', url: 'https://www.google.com' },
  ],
  Program: [
    { name: 'main', korean: '메인', url: 'https://www.google.com' },
    { name: 'reference', korean: '조회', url: 'https://www.google.com' },
    { name: 'satisfy', korean: '만족도 조사', url: 'https://www.google.com' },
    { name: 'subscribe', korean: '구독', url: 'https://www.google.com' },
  ],
  Notice: [
    { name: 'main', korean: '메인', url: null, componentName: 'Notifications' },
  ],
  Setting: [
    { name: 'main', korean: '메인', url: null, componentName: 'SettingScreen' },
  ],
};
